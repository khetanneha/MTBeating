...
if (selectType == 0)
    ...
    d  = load( './exptplots/input_files/coordinates_v2.mat' );
    fn = d.coordinates(:,3);
    xx = d.coordinates(:,1);
    yy = d.coordinates(:,2);
    % -------------------------------------------------------
...
figure(1),...
    ...
    set( gca , 'fontsize' , 24 ),...
    axis equal,...
    export_fig( gcf, [ opath , 'exptXY_', sprintf('%s', filename) , '.pdf'], '-r300' , '-transparent');
%%  ------------------------------------------------------------------------------------------------------------------------------------
...
xlim([ 0 xMaxLabel ] ),...
    print( gcf, [ opath , 'PSI_', sprintf('%s', filename) , '.pdf'], '-dpdf', '-r300' )
...
figure(Fh+1),...
    ...
    % (rad/\mum),...
xlim([ 0 xMaxLabel ] ),...
    export_fig( gcf, [ opath , 'Curv_', sprintf('%s', filename) , '.pdf'], '-r300' , '-transparent');
...
figure( Fh+2 ),...
    ...
    xlim([ 0 Time  ]),...
    ylabel('\phi '),...
    export_fig( gcf, [ opath , 'FreeTipAng_', sprintf('%s', filename) , '.pdf'], '-r300' , '-transparent');
...
export_fig( gcf, [ opath , 'FreeTipXY_', sprintf('%s', filename) , '.pdf'], '-r300' , '-transparent');
...
figure( Fh + 4 ),...
    ...
    ylabel('Frequency'),...
    set( gca, 'fontsize', 18 ),...
    export_fig( gcf, [ opath , 'dsExpt_', sprintf('%s', filename) , '.pdf'], '-r300' , '-transparent');
...
% Fh = computeFFT( xval , MTip(:,1) , MatTanAng , mean(ds) , 1 , filename , opath );
% %% Compute energy on filament
% Fh = computeEnergy( xval , MTip(:,1), MatCurv ,  (ds) , mean(ds) , 2 , filename , opath );
